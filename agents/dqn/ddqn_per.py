...
from utils import PrioritizedReplayBuffer, plot_training_results
...
class DQNAgent:
    ...
    def train_episode(self) -> Tuple[Union[float, int], int]:
        ...
        while not done:
            ...
            batch_transitions = self.replay_buffer.sample(64)
            ...
            sample, importance, _ = batch_transitions
            ...
            with tf.GradientTape() as tape:
                ...
                if self.use_huber_loss:
                    critic_loss = tf.reduce_mean(tf.keras.losses.huber(targets, q_values, delta=1.) * importance)
                else:
                    critic_loss = tf.reduce_mean(tf.square(targets - q_values) * importance)

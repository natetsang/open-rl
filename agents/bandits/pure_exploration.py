...
def run_pure_exploration(n_arms: int, n_episodes: int = 10) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    for i in range(n_episodes):
        ...
        action = np.random.randint(n_arms)  # Select random action
...
if __name__ == '__main__':
    ...
    q, q_history, a_history, r_history = run_pure_exploration(n_arms=arms, n_episodes=10)
    print("Q_values:\n", q)
